...
Once you have Vagrant installed and this repo cloned, start the lab you need:

```bash
# Default (lab-13: Advanced Network Configuration)
vagrant up

# Or specify a different lab
...
# Or rebuild for a specific lab
vagrant destroy -f && LAB=lab-13 vagrant up
```

This deletes both VMs and rebuilds them from scratch. Your lab files on your host computer are not affected.
...
| **VMs start but can't ping each other** | Run `vagrant reload` to restart both VMs. If that fails, `vagrant destroy -f && vagrant up`. |
